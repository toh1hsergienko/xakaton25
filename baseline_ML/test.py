...
Твоя задача — выбрать из этого списка **не более 5 мест**, которые стоит посетить в первую очередь.
Учитывай разнообразие (парки, музеи, памятники, набережные и т.д.), популярность и логичность маршрута.
Верни **только названия мест**, каждое с новой строки. Ничего больше не пиши.
...
def get_llm_recommendations(places_list):
    ...
    try:
        ...
        # Извлекаем названия: каждая строка — одно место
        recommended_names = [line.strip() for line in raw_text.split("\n") if line.strip()]
        return recommended_names
    except Exception as e:
        print(f"⚠️ Ошибка LLM: {e}. Используем ручной выбор.")
        return None
...
def main():
    ...
    print("Список всех доступных мест:")
    for i, (name, _, _, dist) in enumerate(nearby, 1):
        print(f"{i}. {name} ({dist:.2f} км)")
    ...
    selected_places = []
    if llm_recs:
        # Фильтруем только те, что есть в списке
        valid_recs = [name for name in llm_recs if name in names_list]
        if valid_recs:
            print("\n✨ ИИ рекомендует посетить:")
            for i, name in enumerate(valid_recs, 1):
                print(f"{i}. {name}")
            use_llm = input("\nХотите использовать эти рекомендации? (да/нет): ").strip().lower()
            if use_llm in ("да", "yes", "y", "1"):
                selected_places = [(name, lat, lon) for name, lat, lon, _ in nearby if name in valid_recs]
            else:
                llm_recs = None

    # Если не используем LLM — ручной выбор
    if not selected_places:
        print("\nВыберите номера мест через запятую (например: 1,3,5):")
        try:
            selection = input("Ваши выборы: ").strip()
            indices = [int(x.strip()) - 1 for x in selection.split(",")]
            selected_places = [(nearby[i][0], nearby[i][1], nearby[i][2]) for i in indices if 0 <= i < len(nearby)]
        except (ValueError, IndexError):
            print("❌ Неверный ввод. Выход.")
            return

    if not selected_places:
        print("❌ Нет выбранных мест.")
        return

    print(f"\nВы выбрали {len(selected_places)} мест.")

    # Подготавливаем координаты: [текущая позиция] + [выбранные]
